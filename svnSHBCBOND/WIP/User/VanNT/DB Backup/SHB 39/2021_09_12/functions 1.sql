...
begin
          ...
          select count(*) into l_count from (select * from limits
            ...
            ORDER BY lastchange DESC) where rownum = 1;
        if l_count >0 then
...
BEGIN
    ...
    SELECT SUM(NVL(a.INTRATE * (a.todate - a.FROMDATE ) / a.INTBASEDDOFY * a.PARVALUE /100, 0)
            - fn_calc_fee (p.valuedt,
                            ...
                            '',
                            '',
                            a.INTRATE * (a.todate - a.FROMDATE ) / a.INTBASEDDOFY * a.PARVALUE /100,
                            a.INTRATE * (a.todate - a.FROMDATE ) / a.INTBASEDDOFY * a.PARVALUE /100))
     INTO l_totalamt
      FROM INTSCHD a
...
CREATE OR REPLACE 
FUNCTION fn_calc_investrate (p_frdate date, p_orgdate date, p_matdate date, p_orgprice number, p_quoteprice number, p_symbol varchar2, p_category varchar2,p_productid varchar2, p_custodycd varchar2)
 RETURN number
as
    l_return number := 0;
    l_duedate date;
...
begin
    ...
    l_days := p_matdate - p_frdate;
    l_orgdays:= p_matdate - p_orgdate;
    ...
    if p_productid <>'0' and p_productid is not null then
       SELECT  fn_calc_rate_for_buy (p_productid    ,
        ...
        l_receivedamt := round( p_orgprice * (1+ l_rate/100*l_orgdays/l_accrbasis),0);
    else
    select fn_calc_amt(
                    P_DATE=>p_orgdate,
                    P_TODATE=>p_matdate,
...
BEGIN
    ...
    THEN
        ...
        SELECT NVL (SUM (NVL (a.INTRATE * (a.todate - a.FROMDATE ) / a.INTBASEDDOFY * a.PARVALUE /100, 0)
                         - fn_calc_fee (p.valuedt,
                                        '001',
                                        '',
                                        l_symbol,
    ...
    v_result :=
          (v_result  - l_couponreceived ) * (1 - l_feebuyrate / 100)
        + l_fee / l_execqtty
       ;
    l_feerate := 0;
    l_totalfeerate := 0;
...
begin
        ...
        if l_count >0 then
            select to_char(s.txdate,'yyyy-mm-dd')||nvl(s.txtime,'00:00:00'),s.orgconfirmno, o.execqtty into l_txdatetime, l_orgconfirmno, l_execqtty
            from sereqclose s
            left join oxmast o on o.confirmno = s.orgconfirmno
            where s.confirmno = p_confirmno;
        end if;
