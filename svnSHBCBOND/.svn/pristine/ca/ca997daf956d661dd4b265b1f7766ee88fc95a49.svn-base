...
/**----------------------------------------------------------------------------------------------------
 **  FUNCTION: pr_sabackupdata: Backup d? li?u cu?i ngày
 **  Person         Date            Comments
...
/**----------------------------------------------------------------------------------------------------
 **  FUNCTION: pr_sabackupdata2: Backup d? li?u cu?i ngày t? b?ng TBLBACKUP
 **  Person         Date            Comments
...
BEGIN
   ...
   --Xoa cac bang khong phai bang giao dich, can backup
    ...
    -- Back update nh?ng log dã hoàn t?t
...
/**----------------------------------------------------------------------------------------------------
 **  FUNCTION: pr_sabeforebatch: Backup d? li?u cu?i ngày t? b?ng TBLBACKUP
 **  Person         Date            Comments
 **  ThaiTQ         23/12/2020       Created
 ** (c) 2020 by Financial Software Solutions. JSC.
 ** ----------------------------------------------------------------------------------------------------*/
...
BEGIN
    ...
    -- T? ch?i các giao d?ch 8825, 8815, 8814, 8812, 8816, 8818, 0404 dang ch? duy?t
...
/**----------------------------------------------------------------------------------------------------
 **  FUNCTION: pr_saafterbatch: Backup d? li?u cu?i ngày t? b?ng TBLBACKUP
 **  Person         Date            Comments
 **  ThaiTQ         23/12/2020       Created
 ** (c) 2020 by Financial Software Solutions. JSC.
 ** ----------------------------------------------------------------------------------------------------*/
...
as
/**----------------------------------------------------------------------------------------------------
 ** FUNCTION: prc_process_auto_batch_eod: T? d?ng dóng phiên GD.
 **  MODIFICATION HISTORY
 **  Person      Date           Comments
 **  ThaiTQ      23/12/2020     Created
 ** (c) 2020 by Financial Software Solutions. JSC.
...
as
/**----------------------------------------------------------------------------------------------------
 **  FUNCTION: prc_system_notifyevent2fo: X? lý G?i tín hi?u d?ng b? FO cho serviceMIX
 **  Person         Date            Comments
...
as
/**----------------------------------------------------------------------------------------------------
 **  FUNCTION: prc_system_logevent: X? lý t?o log tín hi?u d? g?i cho serviceMIX
 **  Person         Date            Comments
 **  ThaiTQ         2020/11/30      Created
 ** (c) 2020 by Financial Software Solutions. JSC.
 ** ----------------------------------------------------------------------------------------------------*/
...
as
/**----------------------------------------------------------------------------------------------------
 **  FUNCTION: prc_system_jsonnotify: X? lý G?i tín hi?u g?i Email/SMS cho serviceMIX
 **  Person         Date            Comments
    ...
    if l_emailsmsmod <>'Y' then
        ...
        if rec.type = 'S' then
            ...
            if length(rec.email) >=10 and rec.email like '0%' then
                ...
                -- Check n?u d?a ch? email không dúng --> reject luôn không g?i vào serviceMIX
...
PACKAGE utf8nums 
    ...
    c_const_str_common CONSTANT VARCHAR2(50):= 'Thông tin chung';
    c_const_str_not_less_then CONSTANT VARCHAR2(50):= ' không du?c nh? hon ';
    c_const_str_less_then_or_equal CONSTANT VARCHAR2(60):= ' ph?i nh? hon ho?c b?ng ';
    c_const_custtype_custodycd_ic constant varchar2(30):= 'Cá nhân trong nu?c';
    c_const_custtype_custodycd_bc constant varchar2(30):= 'T? ch?c trong nu?c';
    c_const_custtype_custodycd_if constant varchar2(30):= 'Cá nhân nu?c ngoài';
    c_const_custtype_custodycd_bf constant varchar2(30):= 'T? ch?c nu?c ngoài';

    c_const_custodycd_type_c constant varchar2(30):= 'Trong nu?c';
    c_const_custodycd_type_f constant varchar2(30):= 'Nu?c ngoài';
    c_const_custodycd_type_p constant varchar2(30):= 'T? doanh';
    c_const_feeacm_des constant varchar2(1000):= 'Xóa thông tin phí duy trì tài kho?n';
    c_const_idcode_duplicate constant varchar2(1000):= 'Trùng s? dang ký s? h?u, có ti?p t?c giao d?ch ?';
    c_const_alert_pn constant varchar2(1000):= ' phiên ngày ';
    c_const_alert_sr0034 constant varchar2(1000):= 'Xác nh?n s? l?nh SR0034 c?a qu? ';
    c_const_alert_sr0039 constant varchar2(1000):= 'Xác nh?n s? l?nh SR0039 c?a qu? ';
    c_const_alert_pa0001 constant varchar2(1000):= 'Xác nh?n s? l?nh PA0001 c?a qu? ';
    ...
    c_const_alert_state4bank constant varchar2(1000):= 'Xác nh?n tr? l?i ti?n th?a qu? ';
    c_const_alert_vesting constant varchar2(1000):= ' giao d?ch Vesting chua x? lý';
    c_const_alert_vesting_P constant varchar2(1000):= 'Có ';
    c_const_pending_file_import CONSTANT VARCHAR2(1000) :='Ch? duy?t';
    c_const_p_Individual CONSTANT VARCHAR2(1000) :='Cá nhân';
    c_const_p_not_fount_eey_eer CONSTANT VARCHAR2(1000) :='Không xác d?nh du?c doanh nghi?p ho?c cá nhân';
    c_const_p_not_fount_eer_bank CONSTANT VARCHAR2(1000) :='Ngu?i lao d?ng và tài kho?n ngân hàng không cùng d?i lý';
    c_const_p_not_fount_eey_bank CONSTANT VARCHAR2(1000) :='Cá nhân và tài kho?n ngân hàng không cùng d?i lý';

    c_const_p_fund_tpa CONSTANT VARCHAR2(1000) :='Qu?n lý qu? huu trí';
    c_const_pa0009_1_desc1 CONSTANT VARCHAR2(1000) :='Giá tr? phí: ';
    c_const_pa0009_1_desc2 CONSTANT VARCHAR2(1000) :='t?ng giá tr?';
    c_const_pa0009_1_desc3 CONSTANT VARCHAR2(1000) :='Giá tr? t? ';
    c_const_pa0009_1_desc4 CONSTANT VARCHAR2(1000) :=' Ð?n giá tr? ';
    c_const_pa0009_1_desc5 CONSTANT VARCHAR2(1000) :=' T? l? phí: ';
    c_const_pa0009_1_desc6 CONSTANT VARCHAR2(1000) :=' T? ';
    c_const_pa0009_1_desc7 CONSTANT VARCHAR2(1000) :=' Ð?n ';
    ...
    c_const_p_not_fount_fundcodeid CONSTANT VARCHAR2(1000) :='S? tài kho?n ngân hàng và l?nh mua hoán d?i ph?i thu?c cùng m?t qu?';
    c_const_ngung CONSTANT VARCHAR2(1000) :='Ng?ng';
    c_const_kich_hoat CONSTANT VARCHAR2(1000) :='Kích ho?t';
    c_const_custtype_tc CONSTANT VARCHAR2(1000) :='T? ch?c';
    c_const_custtype_cn CONSTANT VARCHAR2(1000) :='Cá nhân';
    c_const_sex_f CONSTANT VARCHAR2(1000) :='N?';
    c_const_sex_m CONSTANT VARCHAR2(1000) :='Nam';
    c_const_sex_o CONSTANT VARCHAR2(1000) :='Khác';
END utf8nums;
/
