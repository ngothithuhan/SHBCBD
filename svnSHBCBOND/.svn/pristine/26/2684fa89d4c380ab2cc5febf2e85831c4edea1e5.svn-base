...
begin
    ...
    if l_limit_total is not null then
        ...
        SELECT
                    GREATEST (l_limit_total - nvl(SUM((nvl(b.QTTY, 0)- nvl(b.RETURN_QTTY , 0) ) * DECODE(fn_get_limitmethod_sell_symbol(p_afacctno, p_symbol), 'F', nvl(b.PARVALUE, 0), 'P', nvl(b.PRICE , 0))), 0), 0)
...
BEGIN
    l_fee:=0 ;
    l_feecalc:= 0 ;
     for vc in (SELECT distinct feetype  FROM FEETYPE  f WHERE EXECTYPE = p_exectype AND (STATUS = 'A' OR PSTATUS LIKE '%A%') and (getcurrdate() between frdate and todate - 1))
         loop
            SELECT fn_calc_fee
                (p_txdate=>p_date ,
...
--  values(nextval('seq_van_test'),'calc invest rate', '');
    ...
    select case when a.intbaseddofy = 'A' then 365 else 360 end
        into l_accrbasis
    from assetdtl a
    where symbol = p_symbol;
    ...
    if p_category = 'C' then
        ...
        select case when nvl(p2.termval ,0) = 0 then a.duedate
                    ELSE ADD_MONTHS( p_orgdate, p2.TERMVAL)
                end
                into l_duedate
        from product p2
            inner join assetdtl a
...
--  insert into van_test (autoid, objname , value)
--  values(nextval('seq_van_test'),'l_duedate', l_duedate);

    --l_days := l_duedate - p_orgdate;
    l_days := l_duedate - l_currdate;

--  insert into van_test (autoid, objname , value)
--  values(nextval('seq_van_test'),'l_days', l_days);

    -- Tính t?ng s? ti?n nh?n
    --select fn_calc_sum_payment(p_symbol, p_productid, to_char(p_orgdate,'dd/mm/yyyy'), p_orgprice, to_char(l_duedate,'dd/mm/yyyy'), p_custodycd, p_qtty, p_acctno)
        --into l_receivedamt FROM dual;
...
--  values(nextval('seq_van_test'),'l_intamt', l_intamt);
    ...
    --LS = Lãi d?u tu * CSTL / (S? ngày * S? ti?n d?u tu)
    --l_return := l_intamt * l_accrbasis / (l_days * p_quoteprice) * 100;
...
BEGIN
    ...
    THEN
        ...
        -- if l_datereceived > l_frdate then
          ...
          SELECT FN_CALC_RATE_FOR_BUY (l_productid, l_startdate, l_frdate, p_orderid)
          INTO l_rate
          FROM DUAL;
         SELECT FN_CALC_RATE_FOR_BUY (l_productid, l_startdate, l_frdate, 'FEE')
          INTO l_feebuyrate
          FROM DUAL;
        v_result :=    l_orgprice*  l_rate * (l_frdate - l_startdate) / 100 / l_cstl    + l_orgprice     ;
       /* SELECT fn_calc_amt_for_buy (l_productid,
                                    l_startdate,
                                    l_frdate,
                                    p_orderid,
                                    l_orgprice,
        ...
        */
        /* else
             SELECT FN_CALC_RATE_FOR_SELL (l_productid, l_datereceived, l_frdate)
               INTO l_rate
               FROM DUAL;
            ...
            THEN
                ...
                SELECT SUM (
                           fn_calc_amt (p_frdate,
                                        ...
                                        p_custodycd)
                           * l_cstl
                           / (l_cstl + l_rate * (b.calldate - p_frdate) / 100)
                           * b.percent
                           / 100)
...
BEGIN
    ...
    THEN
        ...
        SELECT CASE
                   ...
                   THEN
                       l_intrate + p.amplitude - p.feebuy
                   ELSE
                       l_intrate + p.amplitude
               END
          INTO v_result
          FROM productbuydtl p
         WHERE autoid = l_autoidc;
    ...
    THEN
        ...
        SELECT CASE
                   WHEN p.calfee_method = 'F'
                   THEN
                       l_intrate * p.amplitude / 100 - p.feebuy
                   ELSE
                       l_intrate * p.amplitude / 100
               END
          INTO v_result
          FROM productbuydtl p
         WHERE autoid = l_autoidc;
...
BEGIN
    ...
    THEN
        ...
        l_feebuyrate:= 0 ;*/
        SELECT FN_CALC_RATE_FOR_BUY (l_productid, l_startdate, l_frdate, '')
          INTO l_rate
          FROM DUAL;
         SELECT FN_CALC_RATE_FOR_BUY (l_productid, l_startdate, l_frdate, 'FEE')
          INTO l_feebuyrate
...
BEGIN
    ...
    THEN
        ...
        THEN
            ...
            SELECT SUM (  fn_calc_amt (p_frdate,
                                       b.calldate,
                                       p_symbol,
                                       '',
                                       l_parvalue,
...
if l_calpvmethod ='L' then
   ...
   /* SELECT COUNT (1)
    ...
    THEN
      ...
      ELSIF l_types = 'C'
    THEN
        SELECT nvl(l_intrate,1) * p.amplitude
          INTO v_result
          FROM productbuydtl p
         WHERE autoid = l_autoid;
    ELSE
...
BEGIN
    ...
    SELECT COUNT (1)
      INTO l_count
      FROM otp_logs a
     WHERE /*a.code = 'NEGT' AND */a.refid = p_otpkey AND status = 'N';
    ...
    THEN
        ...
        UPDATE otp_logs
         ...
         WHERE /*code = 'NEGT' AND */refid = p_otpkey AND status = 'N';
...
BEGIN
    ...
    select (a.duedate - getcurrdate()), a.symbol into l_termval, l_symbol from assetdtl a where a.symbol in (select symbol from product where autoid = p_productid);
    /*if l_termval = 0 then
...
for rec in (select * from (select * from productselldtl p where p.id = p_productid and fn_cal_days_by_term(p.termcd, p."FROM")<= l_termval and fn_cal_days_by_term(p.termcd, p."TO")>= l_termval) where rownum = 1 )
   loop
        if rec."TYPE"='V' then
        l_return := rec.rate;
        end if;
...
CREATE OR REPLACE 
FUNCTION fn_get_matdate_product (p_productid varchar2)
 RETURN VARCHAR2
...
BEGIN
    ...
    l_return:=to_char(l_result,'dd/mm/yyyy');
    return l_return;
exception
    when others then
        return to_char('01/01/2999','dd/mm/yyyy');
END;
/
