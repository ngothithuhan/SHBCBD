...
DROP PROCEDURE insert_productclsdtl
/

CREATE OR REPLACE 
PROCEDURE insert_productclsdtl (p_clsdtl varchar2,
                            p_autoid number,
...
begin
 ...
 FOR record
                    IN (SELECT REGEXP_REPLACE (TRIM (fil.char_value),
                                   ...
                                   fil_cond
                          FROM (SELECT fn_pivot_string (
                                           REGEXP_REPLACE (p_clsdtl, '~\$~', '|'))
                                           filter_row
                                  FROM DUAL),
                               TABLE (filter_row) fil)
                LOOP
                    INSERT INTO productclsdtl (autoid,
                                                        id,
                                                        termcd,
                                                        "FROM",
                                                        "TO",
                                                       ...
                                                       AMPLITUDE,
                                                        ...
                                                        term,
                                                        STATUS)
                        SELECT SEQ_PRODUCTCLSDTL.NEXTVAL,
                               p_autoid,
                               b.TERMCD,
                               b."FROM",
                               B."TO",
                          ...
                          FROM (SELECT *
                                  FROM (WITH tmp
                                        ...
                                        select *
                                            from (select * from
                                                (select value TERM from tmp t where t.rid = 1) t1,
                                                (select value TERMCD from tmp t where t.rid = 2) t2,

                                                (select value "FROM" from tmp t where t.rid = 3) t3,
                                                (select value "TO" from tmp t where t.rid = 4) t4,
                                                (select value "TYPE" from tmp t where t.rid = 5) t5,
                                            (select nvl(value, 0) RATE from tmp t where t.rid = 6) t6,
                                            (select nvl(value, 0) AMPLITUDE from tmp t where t.rid = 7) t7
...
DROP PROCEDURE insert_productclsdtlmemo
/

CREATE OR REPLACE 
PROCEDURE insert_productclsdtlmemo (p_clsdtl varchar2,
                            p_autoid number,
                            p_action varchar2,
                            p_err_code in out varchar2)
is
 ...
 FOR record
                    IN (SELECT REGEXP_REPLACE (TRIM (fil.char_value),
                          ...
                          FROM (SELECT fn_pivot_string (
                               ...
                               TABLE (filter_row) fil)
                LOOP
                    INSERT INTO productclsdtlmemo (autoid,
                                                        id,
                                                        termcd,
                                                        "FROM",
                                                        "TO",
...
-- sellercash
  ...
  if l_dealeraccount = l_afacctno then
    ...
    -- insert oxmast
        ...
        if l_category='T' then
            l_contractno:=l_confirmno||'.'||l_shortname||'/HÐTP-B';
        else --if l_category in ('O','I')
            l_contractno:=l_confirmno||'.'||l_symbol||'/HDTP-B';
        end if;
     ...
     /*  if l_buyconfirmno is not null then
        ...
        insert into oxmast (
            ...
            (
             ...
             p_idbuyer,--idbuyer
             p_orgprice, --price gia truoc uu dai
             l_orgdate ,-- orgdate ngay mua lan dau
             nvl(l_orgconfirmno,l_confirmno),--orgconfirmno
             l_dealeraccount,-- sbdefacctno
             l_buyconfirmno,--buyconfirmno
             nvl(l_orgprice,p_orgprice), --orgprice
             l_ccpafacctno,--ccpafacctno
             p_promotion,--promotion
             p_renew,--renew
             p_autoclose,--autoclose
             l_contractno,--contarctno
             'N',--Ttkd_profile_stat
             'N',--Bks_profile_stat
             'N',--Appr_stat
             'N',--Sett_stat
         ...
         -- tang secured ngu?i bán
         if l_category in ('O','T') then
             update semast se set se.secured = nvl(se.secured,0)+ p_qtty where se.afacctno = p_acseller and se.symbol=l_symbol;
             l_txdesc:= 'Ban '||p_qtty||' '||l_symbol||'" - HÐ:'||l_contractno;
             -- insert setran tang secured
              INSERT INTO setran
         ...
         if l_category in ('I') then -- tru?ng h?p bán so c?p
             update semast se set se.blockedsip = nvl(se.blockedsip,0)+ p_qtty where se.afacctno = p_acseller and se.symbol=l_symbol;
             l_txdesc:= 'Ban '||p_qtty||' '||l_symbol||'" - HÐ:'||l_contractno;
             -- insert setran tang blockedsip
              INSERT INTO setran
              (txnum, txdate, acctno, txcd, namt, camt, ref, deltd, autoid, acctref, tltxcd, bkdate, trdesc, lvel, vermatching, sessionno, nav, feeamt, taxamt)
              VALUES(p_txnum, p_txdate, l_acctno, '0016', p_qtty, NULL, '1', 'N', seq_setran.NEXTVAL, '', p_tltxcd,
                 getcurrdate(),'' || l_txdesc || '', 2, NULL, NULL, 0, 0, 0);
        end if;
          --insert semast ngu?i mua
          select count(*) into l_count  from semast se  where se.afacctno = p_acbuyer and se.symbol = l_symbol;
          if l_count = 0 then
          INSERT INTO semast
                  (acctno, afacctno, custodycd,codeid, symbol, trade, tradeepr, tradeepe, careceiving, costprice, receiving, blocked, netting, status, pl, lastchange, sending, secured, tradesip, sendingsip, blockedsip, isallowodsip)
     ...
     if p_intadj ='N' then-- l?nh không c?n di?u ch?nh thông tin
            -- tang firmqtty, firmamt oxpost
            update oxpost o set  o.firmqtty = o.firmqtty + p_qtty,
           ...
           --insert SETRAN tang receiving ngu?i mua
           INSERT INTO setran
          (txnum, txdate, acctno, txcd, namt, camt, ref, deltd, autoid, acctref, tltxcd, bkdate, trdesc, lvel, vermatching, sessionno, nav, feeamt, taxamt)
          VALUES(p_txnum, p_txdate, l_acctnobuyer, '0008', p_qtty, NULL, '1', 'N', seq_setran.NEXTVAL, '', p_tltxcd,
             getcurrdate(),'' || l_txdesc || '', 2, NULL, NULL, 0, 0, 0);
            ...
            -- IVTRAN buyer
              ...
              INSERT INTO ivtran
              (txnum, txdate, acctno,codeid,symbol, txcd, namt, camt, ref,srtype, deltd, autoid, acctref, tltxcd, bkdate, trdesc, lvel)
              VALUES(p_txnum, p_txdate, l_cifacbuyer,l_codeid,l_symbol, '0012', l_execamt, NULL, '1','NN', 'N', seq_ivtran.NEXTVAL, '', p_tltxcd,
              getcurrdate,'' || l_txdesc || '', 2);
              if l_feebuyer <>0 then
                l_txdesc:= 'Phi mua '||p_qtty||' '||l_symbol||'" - HÐ:'||l_contractno;
                INSERT INTO ivtran
        ...
        -- insert ivmast ngu?i bán
          select count(1) into l_count
            ...
            INSERT INTO ivtran
              (txnum, txdate, acctno,codeid,symbol, txcd, namt, camt, ref,srtype, deltd, autoid, acctref, tltxcd, bkdate, trdesc, lvel)
              VALUES(p_txnum, p_txdate, l_cifacseller,l_codeid,l_symbol, '0016', l_execamt, NULL, '1','NN', 'N', seq_ivtran.NEXTVAL, '', p_tltxcd,
              getcurrdate,'' || l_txdesc || '', 2);
              if l_feeseller <>0 then
              l_txdesc:= 'Phi ban '||p_qtty||' '||l_symbol||'" - HÐ:'||l_contractno;
                INSERT INTO ivtran
